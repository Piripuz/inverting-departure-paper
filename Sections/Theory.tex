...
\begin{lemma}
  \label{lemma:decreasing-early-int}
...
\begin{lemma}
  \label{lemma:decreasing-late-int}
  Let \(\beta_0 < \beta_1 \in \R\),
  choose two critical early arrival intervals 
  \begin{align*}
    I_0 \in E(\beta_0) && I_1 \in E(\beta_1)
...
The proof for Lemma~\ref{lemma:decreasing-late-int} is analogous to what seen for Lemma~\ref{lemma:decreasing-early-int},
and is thus omitted.

\begin{figure}
  \centering
  \includegraphics[width=.8\textwidth]{decreasing_int}
  \caption{
    Two Critical Early Arrival Intervals for different values of the parameter \(\beta\).
    It can be seen how the CEA interval for the higher \(\beta\) is strictly contained in the one relative to the lower value.
  }
  \label{fig:decreasing-int}
\end{figure}

Figure~\ref{fig:decreasing-int} depicts what proved by Lemma~\ref{lemma:decreasing-early-int}:
two CEA intervals are shown,
for different values of \(\beta\).
As expected,
the interval relative to the higher value of \(\beta\) is contained into the other one.
