...
\begin{lemma}
  ...
  the optimal arrival time can be an on-time arrival (that is, \(t^{opt} = t^*\)) only if the neither the optimal early arrival \(t_e^{opt}\) nor the optimal late arrival \(t_l^{opt}\) are defined.
\end{lemma}
\begin{proof}
