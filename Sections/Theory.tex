...
\begin{definition}\label{def:CEA}
    Given any travel time profile function $tt_a(t_a)$, and \(\beta\),  an interval $[t_e, \bar t_e]$ is termed a critical early arrival (CEA) interval when the following conditions hold:
    \begin{align}
         & tt_a(t_e) + \beta (\bar t_e - t_e)   = tt_a(\bar t_e )\\
    &  tt_a(t_e)  + \beta (t - t_e ) \le  tt_a(t ), \forall t \in [t_e, \bar t_e]\label{eq:concave}
    \end{align}
...
\begin{definition}
    Given any travel time profile function $tt_a(t_a)$, and \( \gamma\), an interval $[\bar t_l, t_l]$ is termed a critical early arrival (CLA) interval when the following conditions hold:
     \begin{align}
         &  tt_a(t_l) + \gamma(\bar t_l - t_l)   = tt_a(\bar t_l )\\
...
Any travelers with the unit early and late arrival costs $\beta$ and $\gamma$ would prefer arriving at time $t_e$ than any time in each CEA interval, and prefer arriving at time $t_l$ than any time in each CLA interval, regardless of their $t^*$. 


\begin{prop}
  \label{prop:into-early-late}
Consider a critical early arrival (CEA) interval $[t_e, \bar t_e]$  under some $\beta$ and $tt_a(t)$ and not contained by any other CEA intervals. Under any desired arrival time $t^* \in [t_e, \bar t_e]$, the minimal early arrival cost (MEAC) is achieved at $t_e$, i.e.,  $C_e^{opt} = C(t_e)$.
\end{prop}
\begin{proof}
Recall that the MEAC $C_e^{opt}$ is defined as $$C_e^{opt} = \min_{t \le t^*} C(t).$$
To show that $C_e^{opt}$ is achieved at $t_e$, it is equivalent to show that the inequality $C(t) \ge C(t_e) $ holds for any $t \le t^*$. 

First, we show that $C(t) \ge C(t_e)$ for any $t \in [t_e, t^*]$. By Definition \ref{def:CEA}, for any $t \in [t_e, \bar t_e]$, we have  
\begin{equation}\label{eq:concave}
    tt_a(t ) \ge tt_a(t_e)  + \beta (t - t_e ).
\end{equation} 
By Equation \eqref{eq:cost_ta}, the trip cost at any arrival time $t \in [t_e, t^*]$ is 
$$C(t) = tt_a(t) + \beta (t^* - t).$$
Using \eqref{eq:concave}, we obtain that 
$$C(t) \ge tt_a(t_e) + \beta (t - t_e ) + \beta (t^* - t) = C(t_e).$$

Second, we show that $C(t) \ge C(t_e)$ for any $t < t_e$ by contradiction. Suppose that there exists some $t< t_e$ such that $C(t) < C(t_e)$. 
\end{proof}
