...
\begin{definition}\label{def:CEA}
    Given any travel time profile function $tt_a(t_a)$ and \(\beta\),  an interval $[t_e, \bar t_e]$ is termed an early arrival (EA) interval when the following condition holds:
...
\begin{definition}
    Given any travel time profile function $tt_a(t_a)$ and \( \gamma\), an interval $[\bar t_l, t_l]$ is termed a late arrival (LA) interval when the following conditions hold:
