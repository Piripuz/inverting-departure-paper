...
It is now possible to take advantage of Lemmas~\ref{lemma:decreasing-early-int},~\ref{lemma:decreasing-late-int}:
the monotonicity of the intervals implies that there exist two functions representing the maximum values that \(\beta\) and \(\gamma\) can assume while keeping a given point into the defined intervals:
\begin{equation}
